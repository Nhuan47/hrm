...
export const ManageEmployeeTable = () => {
  ...
  const handleDragEnd = async (e) => {
    ...
    if (before !== attrId) {
      ...
      cardToTransfer = { ...cardToTransfer, showOnEmployeeTable: 1 };
      ...
      const elWithoutShowOnEmp = copy.filter(
        (a) => +a.showOnEmployeeTable !== 1
      );

      // Get list attribute turn on showOnEmployeeTable
      let itemsTurnOn = copy.filter((a) => +a.showOnEmployeeTable === 1);
      ...
      itemsTurnOn = itemsTurnOn.map((el, index) => ({
        ...el,
        showOnEmployeeTableOrder: index + 1,
      }));
  ...
  const getIndicators = () => {
    return Array.from(
      document.querySelectorAll(`[data-group="employee-table"]`)
    );
  };
  ...
  useEffect(() => {
    if (attributes) {
      let itemFiltered = attributes.filter(
        (attribute) => attribute.showOnEmployeeTable === 1
      );

      itemFiltered = itemFiltered.sort(
        (a, b) =>
          parseInt(a.showOnEmployeeTableOrder) -
          parseInt(b.showOnEmployeeTableOrder)
      );
  ...
  const handleDelete = async (item) => {
    const itemsModified = attributes?.map((a) => {
      if (a.id === item.id) {
        return { ...a, showOnEmployeeTable: 0 };
      } else return a;
    });
