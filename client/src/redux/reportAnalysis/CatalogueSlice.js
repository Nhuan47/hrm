...
import * as api from '@/api/reportAnalysis'
...
export const fetchFolders = createAsyncThunk(
    'report&analysys/fetch-report-folders',
    async (formData, { rejectWithValue }) => {
        try {
            // Call api to fecth report folders
            const { status, data } = await api.getFolders()
            if (status === 200) {
                return data
            } else {
